function [ NL, fmin ] = lagSelectConvert( X, m_outputs, Lmax, nit, mode )
...
c{2} =Lmax;
c{3} = Lm;
        [S, vec, lag, p, m, covmat] = coder.ceval('C:\Users\EricS\Desktop\codegen\lib\lagCovConvert\\lagCovConvert.cpp', c);
        info.vec = vec;
        info.lag = lag; 
        info.p = p;
        info.m = m;
...
for i=1:imax,
    ...
    if i==1,
        %[ S ] = lagCov( [], [], L0, info );
        [S, vec, lag, p, m, covmat] = coder.ceval('C:\Users\EricS\Desktop\codegen\lib\lagCovConvert\\lagCovConvert.cpp', cell([], [], L0));
        info.vec = vec;
        info.lag = lag; 
        info.p = p;
        info.m = m;
