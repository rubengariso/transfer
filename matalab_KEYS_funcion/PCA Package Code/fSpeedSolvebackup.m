...
if strcmp(model.type,'RPCA')
    ...
    x=linspace(1/(1-(lower)), 1/(1-(upper)), n_grid);
    for i=1:n_grid,
        f(i)=fun_curve(1-1/(x(i)));
    end
    f((n_grid+1))=fun_curve(fSpeed_min);
    fVec = [[x 1/(1-fSpeed_min)]; f]';
    [~,I] = sort(fVec(:,1));
    fVec = fVec(I,:);
    figure
    plot(2.*fVec(:,1), fVec(:,2))
    xlabel(txt_label);
    ylabel('Sum of Squared Errors')
    fSpeed_min = 1-1/(fVec(find( fVec(:,2) == (min(fVec(:,2)))),1));
    %plot(1./(1-fVec(:,1)),fVec(:,2))
end
...
switch choice
    ...
    function [ f ] = fun_curve(fSpeed)
        ...
        elseif strcmp(model.type,'MWPCA')
            [model] = mwpcaInitialize(Xt, fSpeed);
